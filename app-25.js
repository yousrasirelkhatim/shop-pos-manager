...
function syncInvoiceTabs(list){
  ...
  if(!days.includes(invoiceDay)){
    const today=dayKey(new Date());
    invoiceDay=days.includes(today)?today:(days[0]||'');
  }
  return {months, days};
}
...
function renderInvoices(){
  ...
  renderInvoiceTabs($('invoiceDays'), days.map(day=>{
    const count=list.filter(sale=>saleDayKey(sale)===day).length;
    return {value:day, label:`${dayTabLabel(day)} · ${count}`};
  }), invoiceDay, day=>{
    invoiceDay=day;
    invoicePage=1;
    renderInvoices();
...
function renderAccountsPanel(){
  ...
  panel.innerHTML=`
    ...
    <div class="accounts-stats">
      ...
      <article class="mini-kpi kpi-card tone-target"><div class="kpi-icon"><svg viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="8" stroke="currentColor" stroke-width="1.8"/><circle cx="12" cy="12" r="3" stroke="currentColor" stroke-width="1.8"/></svg></div><span>تارجت اليوم</span><strong>${dayTarget?qty(dayTarget):'—'}</strong></article>
