...
function renderAccountsPanel(){
  ...
  panel.innerHTML=`
    ...
    <div class="meter-wrap">
      <p class="muted">البيع اليومي الفعلي ${Number(avgDaily||0).toLocaleString('ar-EG',{maximumFractionDigits:1})} منتج في اليوم${dayTarget?` / التارجت ${dayTarget}`:''}</p>
...
async function refresh(){
  ...
  try{
    ...
    }else if(accountsSummary.avg_daily==null||Number(accountsSummary.avg_daily)===0){
      accountsSummary={...invoiceStats,...accountsSummary,avg_daily:invoiceStats.avg_daily};
    }
